...
function isThisAnEmail(stringa:string):boolean {
    let emailValida: RegExp = /^[\w-]+(\.[\w-]+)*@([a-zA-Z0-9-]+\.)+[a-zA-Z]{2,}$/;
...
function whatDayIsIt():string {
  ...
  const giorno:number = data.getDay();
  // mi creo un array con i giorni della settimana scritti in string
  const giorniSettimana:Array<string> = ['Domenica', 'Lunedì', 'Martedì', 'Mercoledì', 'Giovedì', 'Venerdì', 'Sabato'];
...
function rollTheDices(numero:number): { sum:number, values:Array<number> } {
    ...
    for (let i = 0; i < numero; i++) {
      ...
      const numeroEstratto:number = dice();
